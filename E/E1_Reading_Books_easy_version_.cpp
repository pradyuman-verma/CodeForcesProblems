...
void solve(){
    ...
    for0(i, 4){
        ...
        sum[i].pb(0);
        for(auto c : temp[i]){
            sum[i].pb(sum[i].back() + c);
        }
    }
    int ans = INF, xx = sum[3].size(), xy = sum[1].size(), xz = sum[2].size();
    for (int cnt = 0; cnt < min(k + 1, xx); ++cnt) {
		if (k - cnt < xy && k - cnt < xz) {
			ans = min(ans, sum[3][cnt] + sum[1][k - cnt] + sum[2][k - cnt]);
		}
	}
    if(ans == INF) cout << -1 ;
    else cout << ans;
...
/*  - map is like dict in python,
    - use pair not map, it can be sort easily sort(all(x)), for(auto [key, value] : map)
