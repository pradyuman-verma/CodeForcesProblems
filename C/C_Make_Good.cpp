...
#define print(x)             cout << &("NO\0YES"[3 * x]) << endl
#define for0(i, n)           for (auto i : range(n)) //0 based indexing
#define for1(i, n)           for (auto i : range(1ll, n + 1ll)) // yay python, 1 bases indexing
...
int modExpo(int a, int b, int m = mod){
    int res = 1; 
    while (b > 0) {if (b & 1)res = (res * a) % m; a = (a * a) % m; b = b >> 1;} 
    return res;
}
void extendEuclids(int a, int b, int*v) {
    ...
    int x = v[1]; v[1] = v[0] - v[1] * (a / b); v[0] = x;
    return;
